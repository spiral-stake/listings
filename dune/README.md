...
> **All-time counters retired (2026-08-25).** The three top counters (all-time deposited / looped /
> borrowed) and their `result_spiral_alltime` matview + thin display query were removed. `all-time
> deposited = looped − borrowed` is a small difference of two large gross cumulative sums valued
> independently, so at high leverage it is dominated by valuation noise and went **negative**
> (−$2,031), which is nonsensical for "user equity contributed." Dropping the trio also removed the
> matview's ~8 cr/day, cutting the credit burn. The TVL-over-time chart already carries looped /
> borrowed / net over time, so nothing accurate was lost. **General rule kept: every dashboard tile
> must read a matview or be otherwise cheap — never scan raw event tables directly.**
